...
import { getCurrentUser, isAuthenticated, logout } from "./Service/AuthService";
...
const UserDashboard = () => {
  ...
  const [loanStatus, setLoanStatus] = useState("No Application");
  ...
  useEffect(() => {
    const checkAuthAndFetchData = async () => {
      try {
        ...
        try {
          const details = await PersonalDetailsService.getMyPersonalDetails();
          setPersonalDetails(details);
        } catch (err) {
          console.log('Personal details not found:', err);
        }

        // Fetch loan application data
        try {
          const [currentApp, progress, status] = await Promise.all([
            LoanApplicationService.getCurrentLoanApplication().catch(() => null),
            LoanApplicationService.getApplicationProgress().catch(() => null),
            LoanApplicationService.getLoanApplicationStatus().catch(() => "No Application")
          ]);

          setCurrentLoanApplication(currentApp);
          setApplicationProgress(progress);
          setLoanStatus(status);
        } catch (err) {
          console.log('Loan application data not found:', err);
          setLoanStatus("No Application");
        }
        
      } catch (err) {
        console.error('Error fetching user data:', err);
        setError('Failed to load user data');
        // If error fetching data but user is authenticated, show basic info
        const user = getCurrentUser();
        setCurrentUser(user);
      } finally {
        setLoading(false);
  ...
  // Get display name - prefer personal details, fallback to username
  const getDisplayName = () => {
    if (personalDetails?.firstName) {
      return `${personalDetails.firstName}${personalDetails.lastName ? ' ' + personalDetails.lastName : ''}`;
    }
    return currentUser?.username || 'User';
  };

  const getFirstName = () => {
    return personalDetails?.firstName || currentUser?.username || 'User';
  };
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  Hello, {getFirstName().charAt(0).toUpperCase() + getFirstName().slice(1)} 👋
                </h1>
                  ...
                  {loanStatus === "No Application" ? (
                    <div>
                      ...
                      <button 
                        ...
                        className="px-6 py-3 bg-sc-blue-600 text-white rounded-full font-semibold hover:bg-sc-blue-700 transition-colors"
                      >
                    ...
                    <div>
                      ...
                      <div className="mt-4">
                        <span
                          className={`px-4 py-2 rounded-full font-semibold text-lg ${
                            loanStatus === "SUBMITTED" || loanStatus === "PENDING_MAKER_APPROVAL"
                              ? "bg-yellow-100 text-yellow-700"
                              : loanStatus === "APPROVED" || loanStatus === "VERIFIED"
                        ...
                        >
                          {loanStatus.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase())}
                        </span>
                      </div>
                      {currentLoanApplication && (
                        <div className="mt-6 space-y-2">
                          <div className="text-sm text-gray-600">
                            <strong>Application ID:</strong> {currentLoanApplication.applicationId}
                          </div>
                          {currentLoanApplication.loanType && (
                            <div className="text-sm text-gray-600">
                              <strong>Loan Type:</strong> {currentLoanApplication.loanType}
                            </div>
                          )}
                          {currentLoanApplication.amount && (
                            <div className="text-sm text-gray-600">
                              <strong>Amount:</strong> ₹{currentLoanApplication.amount.toLocaleString()}
                            </div>
                          )}
                        </div>
                      )}
                      {/* <div className="mt-6">
                        <button 
                          ...
                          className="flex items-center mx-auto text-sc-blue-600 hover:text-sc-blue-800 font-medium text-sm"
                        >
